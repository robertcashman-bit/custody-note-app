...
/** Dropdown labels (keep in sync with renderer TEMPLATE_OPTIONS). */
const TEMPLATE_UI_OPTIONS = Object.freeze([
...
function generateOfficerEmailBody(fields) {
  ...
  switch (templateType) {
    ...
    case 'request_update_after_delay':
      main =
        `I act for ${client} at ${station} in relation to an allegation of ${offence}.\n\n` +
        'There has been a delay in the matter proceeding. Please advise when the interview / charging decision / release is now expected, and whether the matter remains effective.';
      break;
    case 'bail_details_request': {
      ...
      if (!hasBailDate && !hasBailCond) {
        main = intro + 'Please confirm the bail return date and time, and the bail conditions imposed.';
        ...
        main =
          ...
          'Please confirm the date, time, and any bail conditions.';
      } else {
        main =
          intro +
          `I understand the bail conditions are:\n${bailC}\n\n` +
          'Please confirm the conditions and the bail return date and time.';
      }
      break;
    }
