...
const SUBJECT_SUFFIX = Object.freeze({
  disclosure_confirm_attendance: 'Disclosure request',
...
const ALLOWED_TRANSITIONS = {
  ...
  ready_for_outlook: ['opened_in_outlook', 'cancelled', 'deleted'],
  opened_in_outlook: ['sent_manually', 'cancelled', 'deleted'],
  sent_manually: ['deleted'],
  cancelled: ['deleted'],
  deleted: [],
};
...
function generateOfficerEmailBody(fields) {
  ...
  switch (templateType) {
    case 'disclosure_confirm_attendance':
      main = `I have been given the case of ${client} to cover at ${station} on ${date} in relation to an allegation of ${offence}.\n\nCould you please confirm that the matter remains effective and send disclosure to me at the email address below.`;
      break;
    ...
    case 'confirm_matter_effective':
      main = `I have been asked to attend ${station} in relation to ${client} on ${date}.\n\nPlease could you confirm that the matter remains effective.`;
      break;
    ...
    case 'bail_details_request': {
      hasManyThanks = false;
      const intro = `I am writing in relation to ${client}, who attended ${station} on ${date} in relation to an allegation of ${offence}.\n\nI understand that they were released on police bail.\n\n`;
    ...
    case 'voluntary_interview_confirmation':
      main = `I have been asked to assist ${client} in relation to a voluntary interview at ${station} on ${date}.\n\nPlease could you confirm that the interview remains effective and provide disclosure in advance.`;
      break;
...
function buildOutlookComposeUrl({ toEmail, subject, body }) {
  ...
  const bod = str(body).replace(/\n/g, '\r\n');
  if (to) url.searchParams.set('to', to);
  if (sub) url.searchParams.set('subject', sub);
  if (bod) url.searchParams.set('body', bod);
  return url.toString();
}
...
function validateOpenOutlookFields(draft) {
  const errors = [];
