...
const EMPTY_CLOUD_ALARM_MESSAGE =
  ...
  'Use Re-upload all — do not use Full re-sync while the cloud is empty.';
 ...
 * local_saved → pending → syncing → synced | failed | empty_cloud
...
function deriveSyncPhase({
  ...
  if (lastPushOk === false && ((Number(pendingChanges) || 0) > 0 || (Number(dirtyPushCount) || 0) > 0)) {
    return 'failed';
  }
...
function isSyncStatusHealthy({
  ...
  if ((Number(failedCount) || 0) > 0 || lastError) return false;
  if ((Number(pendingChanges) || 0) > 0 || (Number(dirtyPushCount) || 0) > 0) return false;
  if (lastPushOk === false) return false;
  if (
    detectLocalFullCloudEmpty({
      totalRecords,
      lastPullReceived,
