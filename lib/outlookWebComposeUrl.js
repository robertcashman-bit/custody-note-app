...
function _buildComposeSearch(to, cc, bcc, subject, body) {
...
function buildOutlookComposeUrls(opts) {
  ...
  const body = o.body || '';
  const q = _buildComposeSearch(to, cc, bcc, subject, body);
  return {
...
function _assembleForType(accountType, to, cc, bcc, subject, body, route) {
  const t = _normaliseAccountType(accountType);
  route = String(route || '').toLowerCase();
  if (route === 'work_alt') {
    return buildOutlookComposeUrls({ to, cc, bcc, subject, body }).office365Alt;
  }
  if (route === 'work_primary') {
    return buildOutlookComposeUrls({ to, cc, bcc, subject, body }).office365;
  }
  if (route === 'personal') {
    return buildOutlookComposeUrls({ to, cc, bcc, subject, body }).personal;
  }
  if (t === 'work') {
    return buildOutlookComposeUrls({ to, cc, bcc, subject, body }).office365;
  }
...
function buildOutlookWebComposeUrl(opts) {
  ...
  const route = o.route || '';
  return _assembleForType(accountType, o.to || '', o.cc || '', o.bcc || '', o.subject || '', o.body || '', route);
}
...
function buildOutlookWebComposeUrlWithMeta(opts) {
  ...
  let url = _assembleForType(accountType, to, cc, bcc, subject, body, route);
  if (url.length <= softLimit) {
    return { url: url, truncated: false, reason: null, accountType: accountType };
  }

  const overheadUrl = _assembleForType(accountType, to, cc, bcc, subject, OWA_BODY_FALLBACK_NOTICE, route);
  ...
  url = _assembleForType(accountType, to, cc, bcc, subject, truncatedBody, route);
