...
/* H62 — when present, login_hint tells OWA / login.microsoftonline.com which
   account to use, side-stepping Edge / Outlook-PWA hijacks where the active
   browser session is signed in to a different account (e.g. Gmail). Only
   added for the https:// surfaces; mailto has no such concept. */
function _isLikelyEmail(s) {
  const v = String(s == null ? '' : s).trim();
  if (!v) return false;
  const at = v.indexOf('@');
  return at > 0 && at < v.length - 1 && v.indexOf(' ') < 0;
}

function _buildComposeSearch(to, cc, bcc, subject, body, loginHint) {
  const params = new URLSearchParams();
  params.set('to', String(to == null ? '' : to));
  params.set('cc', String(cc == null ? '' : cc));
  params.set('bcc', String(bcc == null ? '' : bcc));
  params.set('subject', String(subject == null ? '' : subject));
  params.set('body', String(body == null ? '' : body));
  if (_isLikelyEmail(loginHint)) {
    params.set('login_hint', String(loginHint).trim());
  }
  return params.toString();
}

function buildOutlookComposeUrls(opts) {
  const o = opts || {};
  const to = o.to || '';
  const cc = o.cc || '';
  const bcc = o.bcc || '';
  ...
  const body = o.body || '';
  const loginHint = o.loginHint || '';
  const q = _buildComposeSearch(to, cc, bcc, subject, body, loginHint);
  return {
    office365: 'https://outlook.office.com/mail/deeplink/compose?' + q,
    office365Alt: 'https://outlook.office.com/owa/?path=/mail/action/compose&' + q,
    personal: 'https://outlook.live.com/mail/0/deeplink/compose?' + q,
  };
}

function _assembleWeb(host, basePath, to, cc, bcc, subject, body, loginHint) {
  const q = _buildComposeSearch(to, cc, bcc, subject, body, loginHint);
  const sep = String(basePath).indexOf('?') >= 0 ? '&' : '?';
  return 'https://' + host + basePath + sep + q;
}
...
function _assembleForType(accountType, to, cc, bcc, subject, body, route, loginHint) {
  const t = _normaliseAccountType(accountType);
  route = String(route || '').toLowerCase();
  if (route === 'work_alt') {
    return buildOutlookComposeUrls({ to, cc, bcc, subject, body, loginHint }).office365Alt;
  }
  if (route === 'work_primary') {
    return buildOutlookComposeUrls({ to, cc, bcc, subject, body, loginHint }).office365;
  }
  if (route === 'personal') {
    return buildOutlookComposeUrls({ to, cc, bcc, subject, body, loginHint }).personal;
  }
  if (t === 'work') {
    return buildOutlookComposeUrls({ to, cc, bcc, subject, body, loginHint }).office365;
  }
  ...
  /* mailto has no concept of account hint — let the OS pick the registered handler. */
...
function buildOutlookWebComposeUrl(opts) {
  ...
  const route = o.route || '';
  return _assembleForType(accountType, o.to || '', o.cc || '', o.bcc || '', o.subject || '', o.body || '', route, o.loginHint || '');
}
...
function buildOutlookWebComposeUrlWithMeta(opts) {
  ...
  let url = _assembleForType(accountType, to, cc, bcc, subject, body, route, loginHint);
  if (url.length <= softLimit) {
    return { url: url, truncated: false, reason: null, accountType: accountType };
  }

  const overheadUrl = _assembleForType(accountType, to, cc, bcc, subject, OWA_BODY_FALLBACK_NOTICE, route, loginHint);
  ...
  url = _assembleForType(accountType, to, cc, bcc, subject, truncatedBody, route, loginHint);
  return { url: url, truncated: true, reason: 'body_too_long', accountType: accountType };
}

module.exports = {
  buildOutlookComposeUrls,
