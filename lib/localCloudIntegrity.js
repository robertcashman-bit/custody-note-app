...
function buildLocalCloudIntegrityReport(input = {}) {
  ...
  const cloudEmptyProven =
    !!input.pulledFromEpoch && inventoryKnown && inventory === 0 && localActive > 0;
  ...
  if (cloudEmptyProven) {
    discrepancies.push({
      ...
      message: 'Local notes exist but from-epoch inventory proves cloud has 0 — do not wipe local; use Re-upload all',
    });
  }
