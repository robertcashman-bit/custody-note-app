 ...
 * Only after confirmed ack — OR integrity-proven already-present (written:0).
 *
 * alreadyPresentInCloud requires integrityClean (caller must prove cloud holds
 * the sync_id via cloud id set). Never use for empty-cloud / localOnly cases.
 */
function mayClearOutboxEntry(ack = {}) {
  if (!ack || ack.confirmed !== true) return false;
  if (ack.ambiguous === true) return false;
  if (ack.alreadyPresentInCloud === true) {
    return ack.integrityClean === true;
  }
