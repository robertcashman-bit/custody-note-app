...
function resolveMasterKey(deps) {
  ...
  const tryLoadFallback = () => {
    ...
    try {
      ...
      if (isValidMasterKeyHex(fromObfuscated)) return fromObfuscated;
      const plaintext = Buffer.isBuffer(raw) ? raw.toString('utf8').trim() : String(raw || '').trim();
      if (isValidMasterKeyHex(plaintext)) return plaintext;
    } catch (err) {
  ...
  if (safeAvailable) {
    ...
    const fallbackKey = tryLoadFallback();
    if (fallbackKey) {
      return {
        key: fallbackKey,
        quarantinedKeyPath,
  ...
  const fallbackKey = tryLoadFallback();
  if (fallbackKey) {
    return { key: fallbackKey, quarantinedKeyPath: null, created: false, reason: 'fallback_no_safeStorage' };
  }
  if (!allowCreate) {
    return { key: null, quarantinedKeyPath: null, created: false, reason: 'missing' };
  }
