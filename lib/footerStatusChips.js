...
(function (root, factory) {
  ...
  function deriveSyncFooterChip(st) {
    ...
    // Prefer unique case count — dirty+queue for the same record must not double-count.
    var pendingOrDirty = (st.pendingCaseCount != null)
      ? Number(st.pendingCaseCount) || 0
      : (st.health && st.health.pendingCaseCount != null)
        ? Number(st.health.pendingCaseCount) || 0
    ...
    if (isLicenceAuthSyncFailure(st)) {
      return {
        text: /invalid licence/i.test(String(st.lastError || '')) ? 'Invalid licence — fix in Settings' : 'Activate licence to sync',
        variant: 'offline',
        title: licenceAuthSyncRecoveryTitle(st),
        cursor: 'pointer',
      };
    }
    ...
    if (lastPushFailed && pendingOrDirty > 0) {
      return {
        text: pendingOrDirty + ' not confirmed — Fix sync now',
        variant: 'offline',
        title: (st.lastPush && st.lastPush.error ? st.lastPush.error + ' — ' : '') +
          'Local records stay dirty until the cloud confirms a durable write. Click Fix sync now to upload pending records with progress.',
        cursor: 'pointer',
        action: 'fix_sync_now',
      };
    }
   ...
   */
  function isLicenceAuthSyncFailure(st) {
    if (!st) return false;
    if (st.authRequired || st.connectivity === 'auth_required' || st.lastSyncSkipReason === 'auth_required' ||
        st.syncPhase === 'auth_required') {
      return true;
    }
    var err = String(st.lastError || '');
    return /invalid licence/i.test(err) || /licence key is required/i.test(err);
  }

  function licenceAuthSyncRecoveryTitle(st) {
    var err = String(st && st.lastError || '');
    if (/invalid licence/i.test(err)) {
      return 'Invalid licence key — use Email my licence key in Settings → Licence, or paste the key from your purchase email. Sync resumes automatically after activation.';
    }
    return 'Cloud sync needs your licence on this computer. Use Email my licence key in Settings → Licence (Microsoft Store installs start fresh). Sync resumes automatically after activation.';
  }

  function licenceAuthSyncRecoveryHint(st) {
    var err = String(st && st.lastError || '');
    if (/invalid licence/i.test(err)) {
      return 'Cloud sync rejected the licence key on this device. Open Settings → Licence and use Email my licence key (enter your purchase email), or paste the CN-… key from your purchase email. Sync will resume automatically once the key is valid.';
    }
    return 'Cloud sync is paused until this computer has your licence. In Settings → Licence, tap Email my licence key or paste your key. A Microsoft Store install does not copy your licence from another PC — you only need to do this once.';
  }
  ...
  return {
    ...
    isLicenceAuthSyncFailure: isLicenceAuthSyncFailure,
    licenceAuthSyncRecoveryTitle: licenceAuthSyncRecoveryTitle,
    licenceAuthSyncRecoveryHint: licenceAuthSyncRecoveryHint,
  };
});
