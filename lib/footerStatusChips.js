  ...
  function deriveSyncFooterChip(st) {
    ...
    if (rateLimited) {
      ...
      var rlTitle = (st.lastError || 'Too many requests.') +
        (pendingOrDirty > 0 ? ' ' + pendingOrDirty + ' record(s) still waiting to upload — retry after cooldown.' : ' Sync resumes automatically after the cooldown.');
      return {
        text: 'Rate limited — retry in ~' + minsRl + 'm',
        variant: 'offline',
        title: rlTitle,
        cursor: 'pointer',
      };
