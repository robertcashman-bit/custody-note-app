...
/** Quit/crash mid-heal leaves status=running; expire so later cycles can heal again. */
const STALE_HEAL_RUNNING_MS = 30 * 60 * 1000; // 30m
...
/**
 * True when persisted heal status is `running` and lastHealAt is within the
 * stale window. Missing/invalid lastHealAt is treated as stale (not in progress).
 */
function isHealRunningStale(state, now) {
  if (!state || state.status !== 'running') return false;
  const last = Date.parse(String(state.lastHealAt || ''));
  if (!Number.isFinite(last)) return true;
  const t = now != null ? Number(now) : Date.now();
  return t - last >= STALE_HEAL_RUNNING_MS;
}

/** Active heal lock for probe/heal decisions (false when stale or not running). */
function isHealInProgress(state, now) {
  if (!state || state.status !== 'running') return false;
  return !isHealRunningStale(state, now);
}

function probeCooldownActive(lastProbeAt, now) {
  if (!lastProbeAt) return false;
  const last = Date.parse(String(lastProbeAt));
  if (!Number.isFinite(last)) return false;
  const t = now != null ? Number(now) : Date.now();
  return t - last < STALE_CYCLE_PROBE_MS;
}
...
function shouldProbeCloudFromEpoch({
  ...
  const t = now != null ? Number(now) : Date.now();
  const inProgress =
    healState != null ? isHealInProgress(healState, t) : !!healInProgress;
  if (inProgress) return { probe: false, reason: 'heal_in_progress' };
  ...
  // Empty and unknown inventory share the same probe cooldown so max_attempts /
  // inventory=0 cannot reset the cursor and from-epoch-pull every 10s cycle.
  if (inventory === 0) {
    if (probeCooldownActive(lastProbeAt, t)) {
...
function shouldAutoHealEmptyCloud({
  ...
  const inProgress =
    healState != null ? isHealInProgress(healState, t) : !!healInProgress;
  if (inProgress) return { heal: false, reason: 'heal_in_progress' };
