 ...
 * attendance (does not consume a new sequence number). Otherwise peek the next
 * candidate, create, and on conflict bump only past the attempted number
 * (smallest free ≥ expected next) — never jump ahead from a mis-parsed error.
 *
 * @param {object} opts
