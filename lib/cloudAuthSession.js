...
function applyIssuedTokens(licenceData, issued, opts) {
  ...
  } else if (iss.accessToken) {
    // Reissued access token without server expiry: drop any stale tokenExpiresAt
    // from a prior session/synthetic TTL so the new token is not treated as expired.
