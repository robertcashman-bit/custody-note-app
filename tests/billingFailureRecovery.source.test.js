...
describe('billing failed-export recovery', () => {
  it('defines a single failure handler with a recovery action', () => {
    assert.ok(BILLING_SCREEN.includes('function _wfShowInvoiceFailure'), 'recovery helper missing');
    assert.ok(/press .*Send.* again|press Send again|press "Send Bill to QuickFile" again/i.test(BILLING_SCREEN),
      'must tell the user how to retry');
    assert.ok(/Test QuickFile connection/.test(BILLING_SCREEN),
